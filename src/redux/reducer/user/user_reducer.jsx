...
const up_user = createAsyncThunk("/user/update_user",async (data, { rejectWithValue }) => {
    try {
      const token = localStorage.getItem("token")
      const res = await axios.put("/user/update_user",data,{headers: { token }})
      return res.data
    } catch (err) {
      return rejectWithValue(err.response?.data?.mssg || "Error")
    }
  }
)
...
const user_reducer = createSlice({
    ...
    builder.addCase(register_user.pending, (state) => {
      ...
      builder.addCase(up_user.pending, (state) => {
        state.is_loading = true;
        state.error = null;
      })
      builder.addCase(up_user.fulfilled, (state, action) => {
        state.error = null;
        state.is_loading = false;
        state.user=action.payload.user
        localStorage.setItem("user", JSON.stringify(action.payload.user));
      })
      builder.addCase(up_user.rejected, (state, action) => {
        state.error = action.payload;
        state.is_loading = false;
      })
    ...
    export {login_user,register_user,up_user,
      get_history,delete_history,conserve}
